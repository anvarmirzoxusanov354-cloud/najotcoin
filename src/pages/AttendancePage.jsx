...
const AttendancePage = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      let list = [];
      if (studentsData) {
        list = parseGroupStudents(studentsData);
      }
      if (list.length === 0 && groupData) {
        list = parseGroupStudents(groupData);
        if (!list.length && Array.isArray(groupData.data?.students)) list = groupData.data.students;
      ...
      if (!lessonId) {
        const r = await fetch(BASE + '/groups/' + groupId + '/lesson', {
          ...
          body: JSON.stringify({
            ...
            lesson_date: date ? new Date(date).toISOString() : new Date().toISOString(),
            attendances: [],
          }),
        });
        if (r.ok) {
