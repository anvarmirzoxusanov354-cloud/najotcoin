...
const XonalarContent = () => {
  ...
  const handleSave = async () => {
    if (!form.nomi.trim()) { alert("Xona nomini kiriting!"); return; }
    const capacity = parseInt(form.sigimi);
    if (!capacity || capacity < 1) { alert("Sig'imni to'g'ri kiriting (1 dan katta son)!"); return; }

    const token = localStorage.getItem('accessToken');
    const headers = { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' };
    // capacity va max_student ikkalasini ham yuboramiz — API qaysi birini qabul qilsa
    const body = {
      name: form.nomi.trim(),
      capacity: capacity,
    ...
    if (editId !== null) {
      ...
      try {
        ...
        if (res.ok) {
          setRooms(prev => prev.map(r => r.id === editId ? { ...r, name: form.nomi.trim(), capacity } : r));
        } else {
          try {
            const err = await res.json();
            const msg = Array.isArray(err.message) ? err.message.join(', ') : (err.message || `Xatolik: ${res.status}`);
            alert(msg);
          } catch { alert(`Xatolik: ${res.status}`); }
          return;
        }
      ...
      try {
        ...
        if (res.ok) {
          const saved = await res.json();
          const newRoom = saved.data || saved;
          setRooms(prev => [...prev, { id: newRoom.id || Date.now(), name: form.nomi.trim(), capacity }]);
        } else {
          try {
            const err = await res.json();
            const msg = Array.isArray(err.message) ? err.message.join(', ') : (err.message || `Xatolik: ${res.status}`);
