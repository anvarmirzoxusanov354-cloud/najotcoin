...
// Video fayl URL yasash — /files/files/ path ishlatadi
function buildVideoUrl(rawUrl) {
  if (!rawUrl) return '';
  const s = String(rawUrl);
  if (s.startsWith('http')) return s;
  const clean = s.replace(/^\/?(files\/files\/|files\/|file\/|uploads\/|upload\/)?/, '');
...
function VideoUploadModal({ groupId, groupLessons, setGroupLessons, onClose, onUploaded }) {
  var [mode, setMode] = React.useState('file'); // 'file' | 'url'
  ...
  // URL mode state
  var [urlItems, setUrlItems] = React.useState([{ url: '', lessonId: '', videoName: '' }]);
  var [urlError, setUrlError] = React.useState('');
  var [urlSuccess, setUrlSuccess] = React.useState('');
  ...
  function updateUrlItem(idx, field, val) {
    setUrlItems(function(prev) {
      var next = prev.slice();
      next[idx] = Object.assign({}, next[idx], { [field]: val });
      return next;
    });
  ...
  function removeUrlItem(idx) {
    setUrlItems(function(prev) { return prev.filter(function(_, i) { return i !== idx; }); });
  }
  ...
  async function handleUrlUpload() {
    ...
    var valid = urlItems.filter(function(u) { return u.url.trim() && u.lessonId; });
    if (valid.length === 0) {
      setUrlError('URL va darsni kiriting');
      return;
    }
    setUploading(true);
    var token = localStorage.getItem('accessToken');
    ...
    for (var i = 0; i < valid.length; i++) {
      var item = valid[i];
      // URL dagi fayl nomini avtomatik olish
      var autoName = item.videoName.trim() || item.url.split('/').pop() || 'Video';
      try {
        // POST /api/v1/files/group/{groupId}/upload?lessonId={lessonId}
        // JSON body bilan URL yuborish
        var r = await fetch(BASE + '/files/group/' + groupId + '/upload?lessonId=' + item.lessonId, {
          method: 'POST',
          headers: {
        ...
        if (!r.ok) {
          // Agar JSON qabul qilmasa, FormData bilan URL yuborishga urinamiz
          var fd2 = new FormData();
          fd2.append('url', item.url.trim());
          fd2.append('name', autoName);
          var r2 = await fetch(BASE + '/files/group/' + groupId + '/upload?lessonId=' + item.lessonId, {
            method: 'POST',
            headers: { Authorization: 'Bearer ' + token },
            body: fd2,
          });
          if (!r2.ok) {
            ...
            errors.push(autoName + ': ' + r2.status + ' - ' + t2);
          }
        }
      } catch(e) {
        errors.push(item.url + ': ' + e.message);
      }
    }

    setUploading(false);
    if (errors.length > 0) {
      setUrlError('Xatoliklar: ' + errors.join('; '));
    } else {
      setUrlSuccess('Muvaffaqiyatli qo\'shildi!');
      setTimeout(function() { onUploaded(); }, 800);
    }
  }

  var urlCanSubmit = urlItems.some(function(u) { return u.url.trim() && u.lessonId; });
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:1200, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      ...
      <div style={{ position:'relative', background:'white', borderRadius:16, boxShadow:'0 10px 40px rgba(0,0,0,0.18)', width:'100%', maxWidth:660, zIndex:1, overflow:'hidden', display:'flex', flexDirection:'column', maxHeight:'90vh' }}>
        {/* Header */}
        <div style={{ padding:'22px 24px 0', display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:'1px solid #f1f1f5' }}>
          <h2 style={{ margin:0, fontSize:18, fontWeight:700, color:'#1a1a2e' }}>Video qo'shish</h2>
          <button onClick={onClose} style={{ background:'none', border:'none', cursor:'pointer', color:'#9ca3af', fontSize:24, lineHeight:1 }}>×</button>
        </div>

        {/* Mode tabs */}
        <div style={{ display:'flex', gap:0, padding:'0 24px', borderBottom:'1px solid #f1f1f5' }}>
          {[{ key:'file', label:'📁 Fayl yuklash' }, { key:'url', label:'🔗 URL orqali' }].map(function(t) {
            return (
              <button key={t.key} onClick={function(){ setMode(t.key); setUrlError(''); setUrlSuccess(''); }}
                style={{ padding:'12px 20px', border:'none', background:'none', cursor:'pointer',
                  fontWeight:600, fontSize:13, color: mode === t.key ? '#7c4dff' : '#9ca3af',
                  borderBottom: mode === t.key ? '2px solid #7c4dff' : '2px solid transparent',
        ...
        <div style={{ padding:'20px 24px', overflowY:'auto', flex:1 }}>
          {mode === 'file' ? (
                        ...
                        <tr key={idx} style={{ borderBottom:'1px solid #f5f5f7' }}>
                          ...
                          <td style={{ padding:'10px 12px' }}>
                            <button onClick={function(){ removeFile(idx); }}
                              style={{ background:'#fff5f5', border:'none', cursor:'pointer', color:'#ef4444', display:'flex', alignItems:'center', justifyContent:'center', width:28, height:28, borderRadius:6 }}>
            ...
            /* URL mode */
            <div>
              <p style={{ margin:'0 0 16px', fontSize:13, color:'#6b7280' }}>Video URL manzilini kiriting. Darsni tanlang va "Saqlash" tugmasini bosing.</p>
              {urlItems.map(function(item, idx) {
                return (
                  <div key={idx} style={{ display:'flex', gap:10, alignItems:'flex-start', marginBottom:12, flexWrap:'wrap' }}>
                    <input type="url" value={item.url}
                      onChange={function(e){ updateUrlItem(idx, 'url', e.target.value); }}
                      placeholder="https://... .mp4"
                      style={{ flex:2, padding:'9px 12px', borderRadius:8, border:'1.5px solid #e5e7eb', fontSize:13, outline:'none', minWidth:200, boxSizing:'border-box' }} />
                    <select value={item.lessonId}
                      onChange={function(e){ updateUrlItem(idx, 'lessonId', e.target.value); }}
                      style={{ flex:1, padding:'9px 10px', borderRadius:8, border:'1.5px solid #e5e7eb', fontSize:12.5, outline:'none', background:'white', minWidth:140 }}>
                    ...
                    <input type="text" value={item.videoName}
                      onChange={function(e){ updateUrlItem(idx, 'videoName', e.target.value); }}
                      placeholder="Video nomi (ixtiyoriy)"
                      style={{ flex:1, padding:'9px 12px', borderRadius:8, border:'1.5px solid #e5e7eb', fontSize:12.5, outline:'none', minWidth:140, boxSizing:'border-box' }} />
                    {urlItems.length > 1 && (
                      <button onClick={function(){ removeUrlItem(idx); }}
                        style={{ background:'#fff5f5', border:'none', cursor:'pointer', color:'#ef4444', width:34, height:38, borderRadius:8, flexShrink:0 }}>✕</button>
                    )}
                  </div>
                );
              })}
              <button onClick={function(){ setUrlItems(function(prev){ return prev.concat([{ url:'', lessonId:'', videoName:'' }]); }); }}
                style={{ padding:'7px 16px', borderRadius:8, border:'1.5px dashed #c3dafe', background:'#f0f9ff', color:'#3b7cf7', fontSize:12.5, fontWeight:600, cursor:'pointer', marginTop:4 }}>
                + Yana URL qo'shish
              </button>
              {urlError && <div style={{ marginTop:12, padding:'10px 14px', background:'#fee2e2', border:'1px solid #fecaca', borderRadius:8, color:'#dc2626', fontSize:13 }}>{urlError}</div>}
              {urlSuccess && <div style={{ marginTop:12, padding:'10px 14px', background:'#dcfce7', border:'1px solid #bbf7d0', borderRadius:8, color:'#16a34a', fontSize:13, fontWeight:600 }}>✓ {urlSuccess}</div>}
            </div>
          )}
        </div>
        ...
        <div style={{ padding:'14px 24px', display:'flex', alignItems:'center', justifyContent:'flex-end', gap:12, borderTop:'1px solid #f1f1f5', background:'white' }}>
          ...
          {mode === 'file' ? (
            <button onClick={handleUpload} disabled={uploading || files.length === 0 || files.some(function(f){ return !f.lessonId; })}
              style={{ padding:'9px 24px', borderRadius:8, border:'none', fontSize:13, fontWeight:600,
                cursor: (!uploading && files.length > 0 && !files.some(function(f){ return !f.lessonId; })) ? 'pointer' : 'not-allowed',
                background: (!uploading && files.length > 0 && !files.some(function(f){ return !f.lessonId; })) ? '#7c4dff' : '#d1d5db',
                color: (!uploading && files.length > 0 && !files.some(function(f){ return !f.lessonId; })) ? 'white' : '#9ca3af',
                transition:'all 0.15s' }}>
              {uploading ? 'Yuklanmoqda...' : 'Fayllarni yuklash'}
            </button>
          ) : (
            <button onClick={handleUrlUpload} disabled={uploading || !urlCanSubmit}
              style={{ padding:'9px 24px', borderRadius:8, border:'none', fontSize:13, fontWeight:600,
                cursor: (!uploading && urlCanSubmit) ? 'pointer' : 'not-allowed',
                background: (!uploading && urlCanSubmit) ? '#7c4dff' : '#d1d5db',
                color: (!uploading && urlCanSubmit) ? 'white' : '#9ca3af',
                transition:'all 0.15s' }}>
...
export default function GroupDetail(props) {
  ...
  var id = String(props.id || routeId || '');
  var handleBack = props.onBack || function() { navigate('/classes'); };
  var handleHomeworkClick = props.onHomeworkClick || function(hwId) { navigate('/classes/'+id+'/homework/'+hwId); };
  var handleHomeworkCreate = props.onHomeworkCreate || function() { navigate('/classes/'+id+'/homework/create'); };
  ...
  useEffect(function() {
    if (!id || fetchedGroupIdRef.current === id) return;
    ...
    if (props.group) {
      ...
      var groupStudents = parseGroupStudents(props.group);
      if (groupStudents.length > 0) {
        setStudents(function(prev) { return prev.length > 0 ? prev : groupStudents; });
        loadHomeworks(groupStudents);
      }
      setLoading(false);
    } else {
      ...
      Promise.allSettled([
        ...
        if (data) {
          ...
          // Always fetch full teachers list to merge details like photo
          fetch(BASE + '/teachers', { headers })
            .then(function(r) { return r.ok ? r.json() : null; })
            .then(function(tData) {
              var list = [];
              if (tData) {
                list = Array.isArray(tData) ? tData : (tData.data || tData.teachers || []);
              }
              
              var existingTeachers = data.teachers || [];
              if (data.teacher) {
                const mainTeacherExists = existingTeachers.some(t => String(t.id) === String(data.teacher.id));
                if (!mainTeacherExists) {
                  existingTeachers = [data.teacher, ...existingTeachers];
                }
              }

              // Create a details map of teachers from fetched list
              const detailsMap = new Map();
              list.forEach(t => {
                if (t && t.id) {
                  detailsMap.set(String(t.id), t);
                }
              });

              // Merge details from detailsMap into existingTeachers
              const mergedTeachers = existingTeachers.map(et => {
                const fetchedDetails = detailsMap.get(String(et.id));
                if (fetchedDetails) {
                  return Object.assign({}, et, fetchedDetails);
                }
                return et;
              });

              // Add additional matched teachers from list
              list.forEach(t => {
                if (t && t.groups) {
                  const isAssigned = t.groups.some(g => String(typeof g === 'object' ? g.id : g) === String(id));
                  if (isAssigned && !existingTeachers.some(et => String(et.id) === String(t.id))) {
              ...
              if (mergedTeachers.length > 0) {
                setGroup(function(prev) {
                  return prev ? Object.assign({}, prev, { teachers: mergedTeachers }) : prev;
                });
              }
            }).catch(function() {});
        } else {
    ...
    // Robust Students list loading: Try API first, then fall back to props.group
    const loadAllStudents = async () => {
      ...
      if (role !== 'TEACHER') {
        try {
          const r = await fetch(BASE + '/groups/one/students/' + id, { headers });
          if (r.ok) {
            const data = await r.json();
            groupStudents = parseGroupStudents(data);
          }
        } catch {}

        if (groupStudents.length === 0) {
          try {
            const r = await fetch(BASE + '/groups/one/' + id, { headers });
            if (r.ok) {
              const data = await r.json();
              groupStudents = parseGroupStudents(data);
            }
      ...
      if (groupStudents.length === 0) {
        try {
          const r = await fetch(BASE + '/groups/' + id, { headers });
          if (r.ok) {
            const data = await r.json();
            groupStudents = parseGroupStudents(data);
          }
      ...
      if (groupStudents.length === 0 && role !== 'TEACHER') {
        try {
          ...
          if (r2.ok) {
            ...
            groupStudents = matched.map(sg => sg.student || sg).filter(s => s && s.id != null);
          }
        } catch {}
      }

      // Absolute last fallback is props.group
      if (groupStudents.length === 0 && props.group) {
        groupStudents = parseGroupStudents(props.group);
      }

      const seen = new Set();
      const uniqueList = groupStudents.filter(function(s) {
        const sid = s && (s.student?.id || s.student_id || s.studentId || s.id);
        if (!sid) return false;
    ...
    // Homeworks — GET /api/v1/homework/{groupId} va GET /api/v1/homework/all
    function loadHomeworks(groupStudents) {
      setHomeworksLoading(true);
      const role = localStorage.getItem('role');
      const fetches = [
        fetch(BASE + '/homework/' + id + '?page=1&limit=500', { headers }).then(function(r) { return r.ok ? r.json() : null; }).catch(function(){ return null; })
      ];
      if (role !== 'TEACHER') {
        fetches.push(
          fetch(BASE + '/homework/all?page=1&limit=500', { headers }).then(function(r) { return r.ok ? r.json() : null; }).catch(function(){ return null; })
        );
      }

      Promise.allSettled(fetches).then(function(results) {
        var resp = (results[0].status === 'fulfilled' && results[0].value) ? results[0].value : null;
        var allHw = (results[1] && results[1].status === 'fulfilled' && results[1].value) ? results[1].value : null;
        ...
        var hwList = allHw ? (Array.isArray(allHw) ? allHw : (allHw.data || [])) : [];

        var list = [];
        raw.forEach(function(lesson) {
          // Each lesson may have multiple homeworks
          var hwItems = (lesson.homework && lesson.homework.length > 0) ? lesson.homework : [];
          
          if (hwItems.length === 0) return; // skip lessons without homework

          hwItems.forEach(function(hwItem) {
            var hwId = hwItem.id;
            if (!hwId) return;

            // Find matching homework from /homework/all for enriched title
            var matchedHw = hwList.find(function(h) {
              return String(h.id) === String(hwId);
            });

            var hwTitle = hwItem.title || hwItem.name || hwItem.topic || lesson.topic || 'Nomsiz';
            var hwDeadline = hwItem.deadline || hwItem.end_date || hwItem.homework_deadline || hwItem.due_date
              || lesson.deadline || lesson.end_date || lesson.homework_deadline || '';

            if (matchedHw) {
              hwTitle = matchedHw.title || matchedHw.name || matchedHw.topic || hwTitle;
              hwDeadline = matchedHw.deadline || matchedHw.end_date || matchedHw.homework_deadline || matchedHw.due_date || hwDeadline;
            }
            ...
            list.push({
              ...
              lessonId:       lesson.id,
              title:          hwTitle,
              created_at:     hwItem.created_at || lesson.created_at,
              deadline:       hwDeadline,
              lesson_date:    lesson.lesson_date || lesson.created_at || '',
              _existStudents: lesson.existStudentsIngroup,
              _pending:       0,
              _accept:        0,
              _reject:        0,
            });
        ...
        // Set the list immediately so page loads instantly
        setHomeworks(list);
        setHomeworksLoading(false);

        // Create tasks of all homeworks
        const tasks = list.map(item => ({ item }));

        const fetchTask = async (task) => {
          const hwId = task.item.id;
          try {
            // Har status uchun alohida so'rov — server faqat status bilan ishlaydi
            const [pendingJson, rejectedJson, acceptedJson] = await Promise.all([
              fetch(`${BASE}/group/${id}/homework/${hwId}/results?status=PENDING`,  { headers, signal }).then(r => r.ok ? r.json() : null).catch(() => null),
              fetch(`${BASE}/group/${id}/homework/${hwId}/results?status=REJECTED`, { headers, signal }).then(r => r.ok ? r.json() : null).catch(() => null),
              fetch(`${BASE}/group/${id}/homework/${hwId}/results?status=ACCEPTED`, { headers, signal }).then(r => r.ok ? r.json() : null).catch(() => null),
            ]);

            const dedupeStudents = (list) => {
              const seen = new Set();
              return list.filter(x => {
                const sid = x.student?.id || x.student_id || x.studentId || x.user_id || x.id;
                const key = sid ? String(sid) : String(Math.random());
            ...
            const pendingCount = dedupeStudents(parseResults(pendingJson)).length;
            const rejectCount  = dedupeStudents(parseResults(rejectedJson)).length;
            const acceptCount  = dedupeStudents(parseResults(acceptedJson)).length;

            setHomeworks(prev => prev.map(hw => {
              if (String(hw.id) === String(hwId)) {
                return { ...hw, _pending: pendingCount, _accept: acceptCount, _reject: rejectCount };
              }
              return hw;
            }));
          } catch {}
        ...
        const limitConcurrency = async (items, fn, limit) => {
        ...
        // Low concurrency of 3 to run smoothly in the background without choking the network
        limitConcurrency(tasks, fetchTask, 3);
      ...
      .then(function(data) {
        if (data) {
          ...
          var normalized = list.map(function(v) {
            ...
            var fullUrl = buildVideoUrl(rawUrl);
  ...
  if (error || !group) return (
    <div style={{ display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', minHeight:300, gap:12 }}>
      ...
      <button onClick={function() { handleBack(); }}
  ...
  var tList = group.teachers && group.teachers.length > 0
    ? group.teachers
    : (group.teacher ? [group.teacher] : (props.teacherProfile ? [props.teacherProfile] : []));
  if (tList.length > 0) {
    teacherName = tList[0].full_name || tList[0].name || "Noma'lum";
  }
  var courseName = '-';
...
function getVideoFileUrl(photo) {
  if (!photo) return null;
  const p = String(photo);
  if (p.startsWith('http')) return p;
  const clean = p.replace(/^\/?(files\/files\/|files\/|file\/|uploads\/|upload\/)?/, '');
  return 'https://najot-edu.softwareengineer.uz/files/files/' + clean;
}
      ...
      {playingVideo && (
        <div style={{ position:'fixed', inset:0, zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
          ...
          <div style={{ position:'relative', background:'#000', borderRadius:12, overflow:'hidden', width:'100%', maxWidth:800, zIndex:1 }}>
            ...
            {(function() {
              // playingVideo.url already normalized via buildVideoUrl on fetch
              var videoUrl = playingVideo.url || buildVideoUrl(playingVideo.video_url || playingVideo.file_url || playingVideo.path || '');
              if (videoUrl) {
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(300px, 1fr))', gap:20, marginBottom:20 }}>
            ...
            <div style={{ background:'white', borderRadius:12, boxShadow:'0 1px 8px rgba(0,0,0,0.06)', overflow:'hidden' }}>
              ...
              <div style={{ padding:24, display:'flex', gap:20, flexWrap:'wrap' }}>
                {tList && tList.length > 0 ? tList.map(function(t, i) {
                  if (!t) return null;
        ...
        <div>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:24 }}>
            ...
            {activeSubTab === 'Uyga vazifa' && (
              <button onClick={function(){ handleHomeworkCreate(); }}
                  ...
                  {homeworksLoading ? (
                    ...
                    return (
                      <tr key={hw.id || idx} style={{ borderBottom:'1px solid #f5f5f7', cursor:'pointer' }}
                        onClick={function(){ handleHomeworkClick(hw.id); }}
                        onMouseEnter={function(e){ e.currentTarget.style.background='#fafafa'; }}
                        onMouseLeave={function(e){ e.currentTarget.style.background='white'; }}>
                        ...
                        <td style={{ padding:'16px 16px', color:'#3b7cf7', fontWeight:600, cursor:'pointer' }}
                          onClick={function(e){ e.stopPropagation(); handleHomeworkClick(hw.id); }}>
                          {hw.title||hw.name||hw.topic||'Nomsiz'}
                        </td>
                        <td style={{ padding:'16px 16px', textAlign:'center', color:'#374151', fontWeight:600 }}>{students.length || hw._existStudents || 0}</td>
                ...
                <VideoUploadModal
                  ...
                  onUploaded={function(){
                    ...
                    fetch(BASE + '/files/' + id, { headers: { Authorization: 'Bearer ' + token } })
                      ...
                      .then(function(d){
                        if (d) {
                          var list = Array.isArray(d) ? d : (d.data || d.files || d.items || []);
                          var normalized = list.map(function(v) {
                            var rawUrl = v.video_url || v.url || v.file_url || v.path || '';
                            return Object.assign({}, v, {
                              url:   buildVideoUrl(rawUrl),
                              title: v.originalname || v.title || v.name || v.filename || 'Video',
                              size:  v.size_mb ? (v.size_mb * 1024 * 1024) : (v.size || 0),
                            });
                          });
                ...
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                  ...
                  <tbody>
                    {videosLoading ? (
                      ...
                      var videoUrl = v.url; // already normalized on fetch
                      return (
