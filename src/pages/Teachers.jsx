...
const Teachers = () => {
        ...
        if (res.ok) {
          ...
          const mapped = list.map((u, i) => ({
            ...
            labels: u.groups
              ...
              : [],
            guruhIds: u.groups
              ? u.groups.map(g => typeof g === 'object' ? (g.id || g._id) : g).filter(Boolean)
              : [],
            jinsi: u.gender || '',
            selected: false,
            archived: false,
  ...
  const handleSave = async () => {
    ...
    // Filter out inactive/deleted group IDs based on availableGroups
    const activeGroupIds = new Set(availableGroups.map(g => Number(g.id)));
    const filteredGuruhIds = (availableGroups.length > 0 && form.guruhIds && form.guruhIds.length > 0)
      ? form.guruhIds.map(Number).filter(id => activeGroupIds.has(id))
      : (form.guruhIds || []);
    const filteredGuruhlar = form.guruhlar.filter((_, i) => {
      const gid = form.guruhIds[i];
      return gid && activeGroupIds.has(Number(gid));
    });
    ...
    if (editId !== null) {
      ...
      try {
        ...
        if (form.avatarFile) {
          ...
          formData.append('photo', form.avatarFile);
          
          // Tanlangan guruhlarni biriktirish (NestJS formatida groups arrayi)
          if (filteredGuruhIds.length > 0) {
            filteredGuruhIds.forEach(id => {
              formData.append('groups[]', id);
            });
          }

          res = await fetch(`https://najot-edu.softwareengineer.uz/api/v1/teachers/${editId}`, {
            method: 'PATCH',
            headers: { Authorization: `Bearer ${token}` },
            body: formData,
          });
          ...
          if (form.parol) body.password = form.parol;
          if (form.born) body.address = form.born;
          if (filteredGuruhIds.length > 0) body.groups = filteredGuruhIds;
        ...
        if (res.ok) {
          setTeachers(prev => prev.map(t => t.id === editId ? {
            ...
            guruh: filteredGuruhlar[0] || '', labels: filteredGuruhlar, guruhIds: filteredGuruhIds, jinsi: form.jinsi,
          } : t));
        } else {
          try {
      ...
      try {
        ...
        if (form.avatarFile) {
          ...
          // Tanlangan guruhlarni biriktirish
          if (filteredGuruhIds.length > 0) {
            filteredGuruhIds.forEach(id => {
              formData.append('groups[]', id);
            });
          ...
          res = await fetch('https://najot-edu.softwareengineer.uz/api/v1/teachers', {
            method: 'POST',
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        if (res.ok) {
          ...
          const newT = {
            ...
            email: form.email,
            guruh: filteredGuruhlar[0] || '',
            phone: form.phone,
            born: form.born,
            jinsi: form.jinsi,
            created: new Date().toLocaleDateString('ru-RU'),
            coin: 0,
            labels: filteredGuruhlar,
            guruhIds: filteredGuruhIds,
            selected: false,
            archived: false,
          };
