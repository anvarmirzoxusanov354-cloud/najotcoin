...
const CreateHomework = (props) => {
  ...
  const id = props.groupId || routeId;
  const navigate = useNavigate();
  const handleSuccess = props.onSuccess || (() => navigate(`/classes/${id}`));
  const handleBack = props.onBack || (() => navigate(`/classes/${id}`));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!lessonId) {
        try {
          ...
          if (r2.ok) {
            ...
            var list = Array.isArray(d2) ? d2 : (d2.data || d2.lessons || d2.items || []);
            if (list.length > 0) {
              // Har doim eng yangi darsni aniqlash uchun saralaymiz
              list.sort(function(a, b) {
                return (b.id - a.id) || (new Date(b.created_at || 0) - new Date(a.created_at || 0));
              });
              lessonId = list[0].id;
            }
          }
      ...
      const now = new Date();
      const deadline = new Date(now.getTime() + 48 * 60 * 60 * 1000); // aniq 48 soat
      ...
      fd.append('homework_deadline', deadlineISO); // ba'zi API lar shu nom bilan kutadi
