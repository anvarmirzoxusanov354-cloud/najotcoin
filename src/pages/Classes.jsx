...
const Classes = () => {
    ...
    fetch(`${BASE_URL}/groups/all`, { headers })
      ...
      .then(data => {
        ...
        const mapped = list.map(g => ({
          ...
          teacher: (Array.isArray(g.teachers) && g.teachers.length > 0) ? (g.teachers[0].full_name || g.teachers[0].name || '') : '',
          teacherIds: Array.isArray(g.teachers) ? g.teachers.map(t => t.id) : [],
          studentIds: Array.isArray(g.students) ? g.students.map(s => s.id || s) : [],
          students: g.students_count || (Array.isArray(g.students) ? g.students.length : 0),
          active: g.is_active !== false,
        }));
        setGroups(mapped);
      })
      .catch(err => {
        console.error('Groups fetch or map failed:', err);
        setErrorMsg('Guruhlarni yuklashda xatolik: ' + err.message);
      });
  ...
  return (
    <div className="p-4 lg:p-6 bg-[#f1f5f9] h-full flex flex-col relative">
          ...
          <div>
            ...
            <div className="relative">
              <input 
                ...
                className="w-full p-[11px_14px] rounded-[10px] border-[1.5px] border-[#e5e7eb] text-[13.5px] outline-none bg-[#f9fafb] focus:border-[#7c4dff] text-[#1a1a2e] transition-all appearance-none cursor-pointer"
      ...
      {errorMsg && (
        <div style={{ padding: '12px 20px', background: '#fee2e2', color: '#dc2626', borderRadius: '10px', marginBottom: '16px', fontSize: '13px', fontWeight: 600 }}>
          {errorMsg}
        </div>
      )}
